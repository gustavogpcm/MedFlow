...
export async function promptuary(app: FastifyInstance) {
  app.post('/', async (request, reply): Promise<FastifyReply> => {
    const bundleBody: any = request.body

    const resourcesToProcess = bundleBody.entry.filter((item) =>
    ...
    for (const resource of resourcesToProcess) {
      const attendanceData = findAttendance(bundleBody)
      const doctorData = findDoctor(bundleBody, resource)
      ...
      prdParams.nr_prescr_medflow = bundleBody.id
